...
- [주가 조회 불가 종목]에 있는 종목은 실제로 가격 조회를 시도했으나 데이터가 없는 종목입니다. 이 종목에 대해 어떤 가격 수치도 절대 언급하지 마세요. 반드시 "해당 종목의 주가 데이터를 가져올 수 없어요. 데이터 소스에 아직 없거나 일시적으로 조회가 어려울 수 있어요."라고만 안내하세요.

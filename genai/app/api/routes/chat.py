...
def _build_system_prompt(level: int, tendency: str, tickers: list[str],
    ...
    if unavailable_tickers:
        ...
        for t in unavailable_tickers:
            lines.append(f"- {t}: 상장 확인됨 (가격 데이터 없음)")
        price_section += "\n" + "\n".join(lines) + "\n"
...
- [주가 조회 불가 종목]에 있는 종목은 DB에서 상장이 확인된 종목이지만 현재 가격 데이터를 가져올 수 없는 상태입니다. 이 종목의 가격 수치는 절대 언급하지 마세요. 상장 여부를 묻는 질문에는 "상장된 종목이지만 현재 주가 데이터를 가져올 수 없어요. 잠시 후 다시 시도해주세요."라고 안내하세요. 가격 질문에는 "주가 데이터를 가져올 수 없어요. 데이터 소스에 아직 없거나 일시적으로 조회가 어려울 수 있어요."라고 안내하세요.

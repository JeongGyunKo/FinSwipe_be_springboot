...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleMissingParam(MissingServletRequestParameterException e) {
        return ResponseEntity.badRequest().body(Map.of("error", "필수 파라미터 누락: " + e.getParameterName()));
    }

    /** 지원하지 않는 HTTP 메서드 (예: GET /auth/reset-password) → 405 */
    ...
    public ResponseEntity<Map<String, String>> handleMethodNotSupported(HttpRequestMethodNotSupportedException e) {
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
                .body(Map.of("error", "지원하지 않는 HTTP 메서드입니다: " + e.getMethod()));
    }

    /** 잘못된 JSON / Content-Type 불일치 → 400 */
    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<Map<String, String>> handleNotReadable(HttpMessageNotReadableException e) {
        return ResponseEntity.badRequest().body(Map.of("error", "요청 형식이 올바르지 않습니다. JSON과 Content-Type을 확인해주세요."));
    }
